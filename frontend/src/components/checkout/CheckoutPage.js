  ...
  const handleClick = async () => {
    ...
    if (cardValid === true) {
      ...
      for (var i = 0; i < cartData.length; i++) {
        ...
        await fetch("/remove_cart", requestOptionsDelete);

        if (cartData[i].itemCategory == "Mystery Box") {
          ...
          await fetch(
            `/mystery_add_units_sold/${cartData[i].id}`,
            requestUnitSold
          );
        }
        // if item is not obtained from mystery box, then add to unit sold
        else if (!cartData[i].itemName.startsWith("[Mystery Box]")) {
          const requestUnitSold = {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
