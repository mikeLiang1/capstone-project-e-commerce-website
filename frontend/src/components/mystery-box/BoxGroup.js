  ...
  async function boxRequest() {
    ...
    if (response.status !== 200) {
      ...
      for (var ID of Object.keys(data.box_data.Products)) {
          ...
          products.push({"itemName" : productData.data.name, "imageUrl": productData.data.image, "price": productData.data.price, "chance": chance, "routeId": ID, "background": background})
              ...
              {products.map((item, id) => (
                  <div className='outline'>
                      <div className="container">
                          <SmallItemContainer key = {id} itemName = {item.itemName} imageUrl = {item.imageUrl} productRouteId = {item.routeId}>
                          </SmallItemContainer>
                          <Link to ={'/product/' + item.routeId}>
                            <div className ="chance" style= {{ background: item.background}}><p>{item.chance}%</p></div>
                          </Link>
                      </div>
                      <b>RRP: ${item.price}</b>
                  </div>
